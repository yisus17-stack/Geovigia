...
type ApiResult = {
  expediente?: {
    ...
    auditoria_ambiental_y_forestal?: { estatus_legal?: string; dictamen_automatizado?: string; registros_deforestacion_hansen?: number[]; alerta_incendios_historicos?: boolean; registros_incendios_historicos?: number[]; deforestacion_en_periodo_analisis?: boolean }
...
function normalizeSavedResult(response: unknown, summary: AnalysisSummary, images: ApiImage[]): ApiResult {
  ...
  const fires = findValue(audit, ['registros_incendios_historicos', 'anios_incendios_historicos', 'anios_incendio'])
  ...
  return {
    expediente: {
      ...
      auditoria_ambiental_y_forestal: {
        ...
        registros_incendios_historicos: (Array.isArray(fires) ? fires : undefined) as number[] | undefined,
...
function AgromichAnalysisPanel({ huertoId }: { huertoId: string }) {
  ...
  const fireYears = audit?.registros_incendios_historicos ?? []
