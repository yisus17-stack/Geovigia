...
import { deleteHuerto, getHuerto, solicitarAuditoria, type Huerto } from '../../lib/huertos'
...
function HuertaDetailPage() {
  ...
  return <AppLayout breadcrumbCurrent={huerto.nombre}>
    ...
    {!pending && <section className="audit-request-panel"><div><p className="eyebrow">Auditoría</p><h2>Enviar a auditoría</h2><p>Cuando el predio esté listo, envíalo a la cola de auditorías pendientes.</p></div><div><button className="button" type="button" onClick={() => { void requestAudit() }} disabled={requestingAudit}>{requestingAudit ? 'Enviando a auditoría…' : 'Enviar a auditoría →'}</button><small className="audit-pdf-note">La huerta aparecerá en el Segundo paso del panel.</small></div></section>}
