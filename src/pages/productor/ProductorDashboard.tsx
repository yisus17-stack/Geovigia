...
function ProductorDashboard() {
  ...
  return <AppLayout role="productor">
    ...
    <section className="section-header producer-map-heading"><div><p className="eyebrow">Vista territorial</p><h2>Huertas registradas</h2><p>{loading ? 'Cargando polígonos guardados…' : huertos.length ? `El mapa muestra ${huertos.length} huerta(s) guardada(s) en tu cuenta.` : 'Aún no hay huertas registradas.'}</p></div>{firstHuerto ? <Link to={`/productor/huertos/${firstHuerto.id}`}>Ver primera huerta →</Link> : <Link to="/productor/huertos/nuevo">Registrar primera huerta →</Link>}</section>
